...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const matchedAdmin = adminData.data.find(
        (admin) => admin[0] === username && admin[1] === password
      );

      if (matchedAdmin) {
        // If admin, redirect to /adminhome
        navigate("/adminhome");
      } else {
        // If not an admin, check for passenger credentials
        ...
        if (matchedPassenger) {
          ...
          setError("Invalid username/email or password."); // Show error if no match is found
        }
      }
    } catch (err) {
  ...
  return (
    <div className="login-container">
      <nav className="navbar navbar-expand-lg pt-3" style={{ textShadow: "0 0 15px #8c8c8c" }}>
        <div className="container-fluid px-5">
          ...
          <ul className="navbar-nav ms-auto">
            <li className="nav-item"><a href="#" className="nav-link fw-semibold text-light fs-5 px-3">Contact Us</a></li>
            <li className="nav-item"><a href="#" className="nav-link fw-semibold text-light fs-5 px-3">About</a></li>
            <li className="nav-item"><a href="#" className="nav-link fw-semibold text-light fs-5 px-3">Flights</a></li>
            <li className="nav-item">
              <a href="#" className="nav-link fw-semibold text-light fs-5 px-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-instagram fw-medium" viewBox="0 0 16 16">
                  ...
                  <span style={{marginLeft: "3px"}}>@FlyEase.pk</span>
              </a>
            </li>
          </ul>
        </div>
      </nav>
      <div className="w-100 d-flex flex-row justify-content-center align-items-center mt-5 pt-2">
        <div className="d-flex flex-column justify-content-center me-5 pe-5" style={{ width: "35%" }}>
          ...
          <form onSubmit={handleLogin} className="bg-transparent w-100 mt-2 mb-0 pt-2 pb-1" style={{ boxShadow: "none" }}>
            <div className="mb-3">
              <label className="form-label text-light fw-medium">User ID / Email</label>
              <input type="text" className="form-control input-box" value={username}
                onChange={(e) => setUsername(e.target.value)} />
            </div>
            <div className="mb-3">
              <label className="form-label text-light fw-medium">Password</label>
              <input type="password" className="form-control input-box" value={password}
